...
class Application():
    ...
    def generate_log_view(self):
        ...
        # self.logs_view.logs_text = tk.Text(self.logs_view, state='disabled', background='white', foreground='black') #.grid(column=0, row=1, padx=10, pady=5, sticky="w", expand=True)
        # self.logs_view.logs_text.insert(tk.INSERT, "TEST DATA")
    ...
    def generate_scripts_view(self):
        ...
        tk.Label(self.scripts_view.table, text="Activate Logging", font="TkDefaultFont 9 bold").grid(row=0, column=2, padx=10, pady=5, sticky="w")
